  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="relative w-full bg-[#FFF9E6] text-[#2D2825] p-6 sm:p-8 rounded-sm shadow-xl border border-[#E6D5B8] min-h-[480px]"
      >
        ...
        <div className="absolute left-2 sm:left-4 top-4 bottom-4 flex flex-col justify-between py-2 pointer-events-none">
          {Array.from({ length: 14 }).map((_, i) => (
            <div
              key={i}
              className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full bg-[#3D322C] shadow-inner"
              style={{ backgroundColor: section.color }}
            />
          ))}
        </div>
        ...
        <div className="pl-6 sm:pl-10 pr-2">
          ...
          {section.id === 'project' ? (
            !selectedProject ? (
              // Project List View (Matches Wireframe 4)
              <div>
            ...
            <div>
              <div className="flex flex-col sm:flex-row justify-between items-start border-b-2 border-[#3D322C]/20 pb-3 mb-4">
                ...
                {section.date && (
                  <div className="text-xs text-[#5C524C] font-mono mt-1 sm:mt-0">
                    <p><span className="font-bold">Date:</span> {section.date}</p>
                    {section.prodi && <p><span className="font-bold">Prodi:</span> {section.prodi}</p>}
                    {section.tags && <p><span className="font-bold">Tags:</span> {section.tags.join(', ')}</p>}
                  </div>
                )}
              </div>
